    ...
    chrome.storage.local.get(['info', 'data'], (res) => {
        ...
        if(res.data.sizeselect == 'on') {
            ...
            for(let size of document.getElementsByClassName('sizes-pdp-options')[0].children) {
                ...
                if(size.innerText.match(/[0-9]{1,2}[.][0-9]{1}|[0-9]{1,2}/gi)[0] == res.data.size && !size.hasAttribute('selected')) {
                    found = true;
                    size.click();
                    break;
                }
            ...
            if(found === true && res.data.atc == "on") {
                alert('run')
                if(document.getElementsByClassName('btn btn-primary btn-block btn-icon glyphicon-shopping-cart')[0] && document.getElementsByClassName('btn btn-primary btn-block btn-icon glyphicon-shopping-cart')[0].disabled === true) {
                    alert('inner')
